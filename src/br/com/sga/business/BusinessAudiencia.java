...
public class BusinessAudiencia implements IBusinessAudiencia {
	...
	private void validar(Audiencia entidade) throws ValidacaoException{
		...
		if(entidade.getVara() == null || entidade.getVara().trim().equalsIgnoreCase(""))
			throw new ValidacaoException("INFORMAÇÃO OBRIGATORIA - INFORME A VARA!!!");
		if(entidade.getOrgao() == null || entidade.getOrgao().trim().equalsIgnoreCase(""))
			throw new ValidacaoException("INFORMAÇÃO OBRIGATORIA - INFORME O ORGÃO!!!");
		if(entidade.getTipo() == null || entidade.getTipo().trim().equalsIgnoreCase(""))
			throw new ValidacaoException("INFORMAÇÃO OBRIGATORIA - INFORME O TIPO!!!");
		if(entidade.getData_audiencia() == null)
			throw new ValidacaoException("INFORMAÇÃO OBRIGATORIA - INFORME A DATA!!!");
	}
